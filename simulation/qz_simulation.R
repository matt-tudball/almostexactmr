...
for(k in 1:nrow(indices)) {
  ...
  # Test statistic is fixed effect regression of Y-beta*D on Z
  if(ja == 1) W <- NULL
  if(ja == 2) W <- Probdat
  if(ja == 3) W <- MFHapG

  TStat_obs <- test_stat(beta,X=G,W=W)

  TStat_null <- null_dist(R=lperm,beta,Prob,W,Jz,verbose=F)

  # p-value
  out[jc,paste('n',jn,'a',ja,sep='')] <- 1-mean(TStat_obs >= TStat_null$beta)
}
